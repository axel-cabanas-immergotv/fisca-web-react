...
import './Localidades/index.js';
import './Secciones/index.js';
import './Circuitos/index.js';
import './Escuelas/index.js';
import './Mesas/index.js';
...
function adminApp() {
    return {
        ...
        localidadesModule: null,
        seccionesModule: null,
        circuitosModule: null,
        escuelasModule: null,
        mesasModule: null,
        ...
        initializeModules() {
            ...
            this.localidadesModule = Object.assign(window.createLocalidadesModule(), sharedContext);
            this.seccionesModule = Object.assign(window.createSeccionesModule(), sharedContext);
            this.circuitosModule = Object.assign(window.createCircuitosModule(), sharedContext);
            this.escuelasModule = Object.assign(window.createEscuelasModule(), sharedContext);
            this.mesasModule = Object.assign(window.createMesasModule(), sharedContext);
        ...
        async loadCurrentSection() {
            ...
            switch (this.currentSection) {
                ...
                case 'mi-equipo':
                    if (this.hasPermission('users.read')) {
                        // Initialize users module to ensure roles are loaded
                        if (!this.usersModule.roles || this.usersModule.roles.length === 0) {
                            await this.usersModule.loadRoles();
                        }
                        await this.usersModule.loadMyTeam();
                    }
                    break;
                case 'roles':
                ...
                case 'affiliates':
                    ...
                    break;
                case 'localidades':
                    if (this.hasPermission('localidades.read')) {
                        await this.localidadesModule.load();
                    }
                    break;
                case 'secciones':
                    if (this.hasPermission('secciones.read')) {
                        await this.seccionesModule.load();
                    }
                    break;
                case 'circuitos':
                    if (this.hasPermission('circuitos.read')) {
                        await this.circuitosModule.load();
                    }
                    break;
                case 'escuelas':
                    if (this.hasPermission('escuelas.read')) {
                        await this.escuelasModule.load();
                    }
                    break;
                case 'mesas':
                    if (this.hasPermission('mesas.read')) {
                        await this.mesasModule.load();
                    }
                    break;
                case 'page-editor':
                case 'story-editor':
