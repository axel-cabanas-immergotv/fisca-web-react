    ...
    return {
        ...
        edit(storyId) {
            ...
            if (storyId) {
                ...
                // Initialize dropzones after DOM is ready
                setTimeout(() => {
                    this.initializeFeaturedImageDropzone();
                }, 500);
            }
        },
        ...
        async loadForEditing(storyId) {
            ...
            try {
                // Clean up existing dropzone before loading new story
                if (this.featuredImageDropzone) {
                    const container = document.getElementById('featured-image-dropzone');
                    if (container) {
                        container.innerHTML = '';
                    }
                    this.featuredImageDropzone = null;
                }
                ...
                if (data.success && data.data) {
                    ...
                    setTimeout(() => {
                        ...
                        // Initialize dropzones and categories after DOM is ready
                        setTimeout(async () => {
        ...
        initializeFeaturedImageDropzone() {
            if (typeof createSimpleDropzone === 'undefined') {
                console.warn('Dropzone utility not loaded');
                return;
            }

            const container = document.getElementById('featured-image-dropzone');
            if (!container) {
                console.error('Featured image dropzone container not found');
                return;
            }

            // Destroy existing dropzone if it exists
            if (this.featuredImageDropzone) {
                // Clean up existing dropzone
                container.innerHTML = '';
            }
            ...
            // Create new dropzone
            this.featuredImageDropzone = createSimpleDropzone('featured-image-dropzone', {
                accept: 'image/*',
                maxFiles: 1,
                maxFileSize: 10 * 1024 * 1024, // 10MB
                uploadUrl: '/api/upload/image',
                folder: 'images/stories/featured',
                showPreview: true,
                previewWidth: 200,
                previewHeight: 150,
                initialValue: this.currentStory.featured_image || null,
                onSuccess: (result) => {
                    if (result) {
                        self.currentStory.featured_image = result.url;
                        self.showNotification('Featured image uploaded successfully!', 'success');
                    } else {
                        self.currentStory.featured_image = '';
                        self.showNotification('Featured image removed.', 'info');
                    }
                    ...
                    if (self.$nextTick) {
                        self.$nextTick(() => {
                            // Trigger reactivity
                            if (self.$dispatch) {
                                self.$dispatch('featured-image-changed', { url: self.currentStory.featured_image });
                            }
                        });
                    }
                },
                onError: (error) => {
                    ...
                    self.showNotification('Upload error: ' + error, 'error');
                }
            });
        }
    };
